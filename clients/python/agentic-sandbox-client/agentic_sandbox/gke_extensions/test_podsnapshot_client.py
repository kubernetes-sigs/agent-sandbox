...
from agentic_sandbox.gke_extensions.podsnapshot_client import PodSnapshotSandboxClient, SnapshotPersistenceManager
...
class TestPodSnapshotSandboxClient(unittest.TestCase):
    ...
    def test_snapshot_controller_ready_self_installed(self, mock_v1_class):
        """Test snapshot_controller_ready for self-installed scenario."""
        logging.info("Starting test_snapshot_controller_ready_self_installed...")
        mock_v1 = MagicMock()
        mock_v1_class.return_value = mock_v1

        # Mock pods in gps-system
        mock_pod_controller = MagicMock()
        mock_pod_controller.metadata.name = 'pod-snapshot-controller'
        mock_pod_controller.status.phase = 'Running'
        ...
        mock_pods = MagicMock()
        mock_pods.items = [mock_pod_controller, mock_pod_agent]
        ...
        mock_v1.list_namespaced_pod.assert_called_with(SNAPSHOT_NAMESPACE_SELF_INSTALLED)
        logging.info("Finished test_snapshot_controller_ready_self_installed.")
    ...
    @patch('agentic_sandbox.gke_extensions.podsnapshot_client.client.CoreV1Api')
    def test_snapshot_controller_ready_not_ready(self, mock_v1_class):
        """Test snapshot_controller_ready when not ready."""
        logging.info("Starting test_snapshot_controller_ready_not_ready...")
        mock_v1 = MagicMock()
        mock_v1_class.return_value = mock_v1
    ...
    def test_checkpoint_controller_not_ready(self):
        ...
        self.assertEqual(result.execution_result.exit_code, 1)
        self.assertIn('test-trigger', result.trigger_name)
        self.assertIn('Snapshot controller is not ready', result.execution_result.stderr)
    ...
    def test_checkpoint_no_pod_name(self):
        ...
        self.assertEqual(result.execution_result.exit_code, 1)
        self.assertIn('test-trigger', result.trigger_name)
        self.assertIn('Sandbox pod name not found', result.execution_result.stderr)
    ...
    def test_checkpoint_timeout(self, mock_custom_class, mock_watch_class):
        ...
        self.assertEqual(result.execution_result.exit_code, 1)
        self.assertIn('timed out', result.execution_result.stderr)
