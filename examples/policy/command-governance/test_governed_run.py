...
def test_denied_env_split_string_fails_closed():
    ...
    assert is_denied("env -S 'rm --recursive --force /'") is True
...
def test_denied_expansion_generated_executable_fails_closed(command):
    ...
    or a glob rather than being a literal spelling can't be statically
    classified, so it's denied outright rather than evaluated."""
    assert is_denied(command) is True
