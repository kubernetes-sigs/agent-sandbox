...
func TestClaimLatencyRecording(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ClaimStartupLatency.WithLabelValues(tc.launchType, "test-pool").Observe(1000)
			...
			ClaimControllerStartupLatency.WithLabelValues(tc.launchType, "test-pool").Observe(1000)
