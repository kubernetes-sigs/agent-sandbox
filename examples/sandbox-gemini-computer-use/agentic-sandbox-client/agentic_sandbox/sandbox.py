...
class Sandbox:
    ...
    def __enter__(self) -> 'Sandbox':
        ...
        w = watch.Watch()
        print("Watching for Sandbox to be created and become ready...")
        sandbox_object = None
        for event in w.stream(
            ...
            field_selector=f"metadata.name={self.claim_name}", # Still need to find the Sandbox object by its name
            timeout_seconds=180
        ):
        ...
        if not sandbox_object: # Changed from self.sandbox_name to sandbox_object
            self.__exit__(None, None, None)
            raise TimeoutError("Sandbox did not become ready within the 180-second timeout period.")

        # Now that we have the Sandbox object, get its selector
        pod_selector = sandbox_object['status']['selector']
        print(f"Found Sandbox selector: {pod_selector}")

        # Watch for the actual pod using the correct selector
        w = watch.Watch()
        print("Watching for Sandbox Pod to become ready...")
        pod_name = None
        for event in w.stream(
            ...
            pod_status = pod_object.status.phase
            if pod_status == 'Running':
                pod_name = pod_object.metadata.name
                w.stop()
                print(f"Sandbox Pod {pod_name} is running.")
                break
        
        if not pod_name:
            self.__exit__(None, None, None)
            raise TimeoutError("Sandbox Pod did not become ready within the 180-second timeout period.")

        self.sandbox_name = pod_name # Update sandbox_name to be the actual pod name
