...
**Which claims this applies to — all of them, and both served API versions.** The resolver keys on the **template name**, which is the one handle every claim shape carries; it is deliberately not keyed on the warm pool.

In `v1beta1`, `warmPoolRef` is `+required` and there is no `templateRef` — the template is reached *through* the pool. `v1alpha1` is still served (`served: true, storage: false`) and has the opposite shape: `templateRef` is the required field and `warmpool` is an optional policy that may be `none`, `default`, or a specific pool name. The conversion webhook maps all three onto a `v1beta1` `warmPoolRef`: a specific pool passes through unchanged, while `none` and `default` synthesize `shadow-pool-<templateRef.Name>`. So the stored object always has a `warmPoolRef`, and the template name is always recoverable from it — it is either the pool's own template or the suffix after the `shadow-pool-` prefix. Fleet resolution therefore needs no per-version branch, and no claim shape bypasses it.

Two `v1beta1` spec fields force a cold start: `env`, because the warm pods were started without those variables, and `volumeClaimTemplates`, because warm pods have no such volumes. A claim carrying either still names a warm pool and is still served on the resolved cluster — the controller cold-starts it from that pool's `SandboxTemplate` instead of adopting a warm replica. `v1alpha1`'s `warmpool: none` is the explicit form of the same thing: never adopt, always create fresh. All of these still resolve to a cluster, because the `SandboxTemplate` they cold-start from has to exist *there*, which is exactly what the assignment guarantees.
...
- **every target is a whole number**,
