...
func TestSandboxClaimCreationMetric(t *testing.T) {
	...
	t.Run("Cold Start", func(t *testing.T) {
		...
		// Verify metric
		val := testutil.ToFloat64(asmetrics.SandboxClaimCreationTotal.WithLabelValues("test-template", asmetrics.LaunchTypeCold, "", "not_ready"))
		if val != 1 {
	...
	t.Run("Warm Start", func(t *testing.T) {
		...
		// Verify metric
		val := testutil.ToFloat64(asmetrics.SandboxClaimCreationTotal.WithLabelValues("test-template", asmetrics.LaunchTypeWarm, "test-pool", "ready"))
		if val != 1 {
			t.Errorf("expected metric count 1, got %v", val)
		}
	})
	...
	t.Run("Warm Start Not Ready", func(t *testing.T) {
		...
		// Verify metric
		val := testutil.ToFloat64(asmetrics.SandboxClaimCreationTotal.WithLabelValues("test-template", asmetrics.LaunchTypeWarm, "test-pool", "not_ready"))
		if val != 1 {
			t.Errorf("expected metric count 1, got %v", val)
		}
	})
