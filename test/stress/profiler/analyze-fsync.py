...
def parse_fsync_log(path, node, run_date):
    ...
    for line in open(path, errors="replace"):
        ...
        if m:
            ts = datetime.combine(
                run_date, datetime.strptime(m.group(1), "%H:%M:%S").time(),
                tzinfo=timezone.utc)
            continue
        if ts is None:
            continue
...
def load(con, artifacts_dir):
    ...
    con.execute("""CREATE OR REPLACE VIEW deltas AS
        ...
        FROM d LEFT JOIN phases p
          ON prev_ts + (ts - prev_ts)/2 BETWEEN p.start_ts AND p.end_ts
    ...
    dump(con, """
        ...
        SELECT c.comm, c.key, c.n AS calls,
               round(b.us / nullif((SELECT sum(n) FROM calls x WHERE x.comm = c.comm), 0), 0)
                 AS mean_us_per_call
        FROM calls c JOIN blocked b USING (comm)
        WHERE c.n > 0 ORDER BY c.comm, calls DESC
    """)
