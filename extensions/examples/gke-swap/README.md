...
This means that pods with larger memory requests will be allocated more swap space (rationed proportionally), while pods with very small requests might not get enough swap to be useful. You can refer to `test/e2e/extensions/chromesandbox_density_test.go` for more details.
...
- A compiled performance results table is written to [`perf_test_summary.md`](perf_test_summary.md).