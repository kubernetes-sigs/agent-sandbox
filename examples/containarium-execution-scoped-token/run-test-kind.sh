...
# concurrent run is still mid-test with. Refuse rather than guess which case
# it is — every name below is one this script is about to create or has
# already created labeled with its own RUN_ID, so any hit here belongs to
# someone else.
COLLIDING=""
for r in \
...
if [ -n "${COLLIDING}" ]; then
  ...
  echo "       this script. Proceeding could let this run's cleanup() delete" >&2
  echo "       resources it did not create, or interfere with a run that is" >&2
  echo "       still using them. Wait for that run to finish, delete its" >&2
  echo "       leftovers by hand, or set NAMESPACE to an unused one." >&2
  exit 1
fi
