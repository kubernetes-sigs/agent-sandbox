...
func (ws *WatchSet) Subscribe(ctx context.Context, gvr schema.GroupVersionResource, filter ObjectFilter) *Subscription {
	watchNamespace := ""
	if filter.Namespace != "" {
...
func (rw *ResourceWatch) subscribe(filter ObjectFilter) *Subscription {
	rw.mu.Lock()
	defer rw.mu.Unlock()
