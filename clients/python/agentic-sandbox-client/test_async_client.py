...
from agentic_sandbox.async_client import AsyncSandboxClient
...
async def main(
    ...
    """
    Tests the async sandbox client by creating a sandbox, running a command,
    and then cleaning up.
    """

    print(
        f"--- Starting Async Sandbox Client Test (Namespace: {namespace}, Port: {server_port}) ---"
    )
    if gateway_name:
    ...
    try:
        ...
        async with AsyncSandboxClient(
            ...
            assert (
                sandbox.annotations is not None
            ), "Sandbox annotations were not stored on the client"
            ...
            if pod_name_annotation:
                ...
                assert (
                    sandbox.pod_name == pod_name_annotation
                ...
                assert (
                    sandbox.pod_name == sandbox.sandbox_name
            ...
            result = await sandbox.run(command_to_run)
            ...
            await sandbox.write(file_path, file_content)

            print(f"Reading content from '{file_path}'...")
            read_content = (await sandbox.read(file_path)).decode("utf-8")
            ...
            list_files_result = await sandbox.run("ls -la /app")
            print(list_files_result.stdout)

            print("\n--- Printing environment variables ---")
            env_result = await sandbox.run("env")
            print(env_result.stdout)

            print("--- Introspection Tests Finished ---")

    except Exception as exc:
        print(f"\n--- An error occurred during the test: {exc} ---")
        # The __aexit__ method of the Sandbox class will handle cleanup.
    finally:
        print("\n--- Async Sandbox Client Test Finished ---")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test the async Sandbox client.")
    parser.add_argument(
        "--template-name",
