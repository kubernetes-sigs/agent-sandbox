...
func main() {
	...
	flag.BoolVar(&enablePprof, "enable-pprof", false,
		"Enable pprof profiling endpoint (/debug/pprof/profile) on the metrics server.")
	...
	if enablePprof || enablePprofDebug {
		...
		if enablePprofDebug {
			setupLog.Info("pprof debug endpoints enabled", "debug", enablePprofDebug)
			runtime.SetBlockProfileRate(1)     // 1 = sample all events
			runtime.SetMutexProfileFraction(1) // 1 = sample all events
