...
These fields will be merged with any existing metadata defined in the `SandboxTemplate`. The values in `SandboxClaim` will take precedence in case of conflicts, allowing specific claims to override template defaults.
...
1.  **Sandbox Metadata**: The controller will take labels/annotations from the `SandboxTemplate` (if any are applicable to the Sandbox itself) and merge them with `SandboxClaim.spec.sandboxMetadata`. The result is set on `Sandbox.metadata`.
