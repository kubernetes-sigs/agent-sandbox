# User Guide: Protecting Agentic Sandboxes with Anthos Policy Controller

## 1. Overview

This guide provides step-by-step instructions for configuring a Anthos Policy Controller on a Kubernetes cluster. The goal of this policy is to **prevent any user or process from granting new permissions to a ServiceAccount that is actively being used by a custom `Sandbox` resource.**
...
**How it Works:**  
The policy intercepts all `RoleBinding` and `ClusterRoleBinding` creation requests. If the request targets a `ServiceAccount` that is referenced by a running `Sandbox`, Anthos Policy Controller will **block the request and return an error**.
...
- `kubectl` access to a Kubernetes **Standard** or **Autopilot** cluster with permissions to install Helm charts.
...
### Step 1: Install [Anthos Policy Controller](https://cloud.google.com/kubernetes-engine/enterprise/policy-controller/docs/how-to/installing-policy-controller)
...
# Disable Anthos Policy Controller Feature
