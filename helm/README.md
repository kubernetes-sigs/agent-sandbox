...
| `controller.watchNamespace` | Namespace(s) to watch; comma-separated for multi-namespace mode. Falls back to `WATCH_NAMESPACE` env var. Empty means cluster-scoped. When set, the chart injects `--leader-election-namespace=<release-namespace>`. ClusterRoles and ClusterRoleBindings are not rendered, and NOTES leaves workload and leader-election Roles and RoleBindings to advanced users. See [configuration](../docs/configuration.md#helm-behavior) for details. | `""` |
