# Managed Sandbox Example — Remaining Work

Running tracker for the examples-first managed sandbox prototype. Update as
items land. See also [controllers/managedsandbox/](controllers/managedsandbox/),
[internal/pool/](internal/pool/), and [pod-agent/](pod-agent/).
...
2. Example controller picks/creates pool pod, mints session token Secret, dials pod-agent gRPC, calls `CreateSandbox`.
...
- Example CRD: `Spec.Image`, `Spec.Workspace`, `Status.Host`, `Status.SSHHost`, `Status.SSHPort`, `Status.SSHSecretName`, `Status.Endpoints`.
- Example controller: reconciles `ManagedSandbox` resources under the example API group.
