...
func TestApplyConfigMapData_CLIWinsOverConfigMap(t *testing.T) {
	data := map[string]string{"sandbox-concurrent-workers": "200"}
	...
	if len(applied) != 0 {
		t.Errorf("expected no overrides (CLI wins over ConfigMap), got %v", applied)
	}
	if workers != 150 {
		t.Errorf("workers = %d, want 150 (CLI wins)", workers)
	}
}
