...
var (
	...
	SandboxClaimCreationTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of SandboxClaims created, labeled by sandbox template, launch type, warmpool name, and pod condition.",
		},
		[]string{"sandbox_template", "launch_type", "warmpool_name", "pod_condition"},
	)
)
...
func RecordSandboxClaimCreation(templateName, launchType, warmPoolName, podCondition string) {
	SandboxClaimCreationTotal.WithLabelValues(templateName, launchType, warmPoolName, podCondition).Inc()
}
