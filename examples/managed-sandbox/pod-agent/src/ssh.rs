...
use crate::worker_client::worker_proto::{
    exec_response::Kind as ExecRespKind, open_shell_request::Kind as ShellReqKind,
    open_shell_response::Kind as ShellRespKind, worker_service_client::WorkerServiceClient,
    ExecRequest, OpenShellRequest, OpenShellResize, OpenShellStart,
};
use crate::Registry;
...
impl russh::server::Handler for SshSession {
    ...
    async fn shell_request(
        ...
        tokio::spawn(async move {
            if let Err(e) = run_shell_session(worker_sock, rx, handle.clone(), channel).await {
    ...
    async fn exec_request(
        ...
        let uid_for_task = uid.clone();
        tokio::spawn(async move {
            if let Err(e) = run_exec_session(worker_sock, command, handle.clone(), channel).await {
                tracing::warn!(uid = %uid_for_task, error = %format!("{e:#}"), "exec session ended");
                let _ = handle
                    .data(channel, CryptoVec::from(format!("exec error: {e}\r\n")))
                    .await;
                let _ = handle.close(channel).await;
            }
        });
...
async fn run_exec_session(
    ...
    while let Some(msg) = stream.message().await? {
        match msg.kind {
            Some(ExecRespKind::Stdout(bytes)) => {
                if handle.data(channel, CryptoVec::from(bytes)).await.is_err() {
                    break;
                }
            }
            Some(ExecRespKind::Stderr(bytes)) => {
                if handle
                    .extended_data(channel, 1, CryptoVec::from(bytes))
                    .await
                    .is_err()
                {
                    break;
