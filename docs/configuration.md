...
* `--sandbox-claim-warm-candidate-grace-period` (default: `2s`): How long a newly created SandboxClaim waits for a warm pool candidate to report a Pod IP before falling back to cold creation. Raise this on clusters where warm pool members take longer than 2s to report a Pod IP (e.g. slow CNI, image pull, or virtual-kubelet status push) to avoid unnecessary cold creations during claim bursts. Must be a positive duration.
