...
// pprof-apiserver-<phase>.pb.gz. Synchronous; run it in a goroutine
// alongside the phase. Analyze with: go tool pprof -top <file>.
func (p *apiserverProfiler) CaptureCPUProfile(ctx context.Context, phase Phase, delay, duration time.Duration) {
	...
	path := filepath.Join(p.outputDir, fmt.Sprintf("pprof-apiserver-%s.pb.gz", phase))
