...
For cluster-wide defaults and egress by domain name (FQDN) you still can use `Managed` mode. The [Network Policy API](https://network-policy-api.sigs.k8s.io/) `ClusterNetworkPolicy` works together with Kubernetes `NetworkPolicy`: Admin-tier rules are evaluated before the template-managed policy, `Pass` rules delegate to it, and Baseline-tier rules apply to pods it does not select. [examples/network-policy-api-sandbox](../../network-policy-api-sandbox/README.md) keeps the templates `Managed` and adds a default deny plus FQDN allowlists on top, using [kube-network-policies](https://kube-network-policies.sigs.k8s.io/) (the SIG Network reference implementation, which can run alongside any CNI) or any other implementation of the API.
