...
func TestReconcileEvents(t *testing.T) {
	...
	newSandbox := func(spec sandboxv1alpha1.SandboxSpec) *sandboxv1alpha1.Sandbox {
		sb := &sandboxv1alpha1.Sandbox{}
		sb.Name = sandboxName
		sb.Namespace = sandboxNs
		sb.UID = sandboxUID
	...
	minimalSpec := sandboxv1alpha1.SandboxSpec{
		PodTemplate: sandboxv1alpha1.PodTemplate{
			Spec: corev1.PodSpec{
				Containers: []corev1.Container{{Name: "c"}},
			},
	...
	t.Run("SandboxExpired emitted after cleanup with Retain policy", func(t *testing.T) {
		...
		expiredSpec := sandboxv1alpha1.SandboxSpec{
			PodTemplate: sandboxv1alpha1.PodTemplate{
				Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "c"}}},
			},
			Lifecycle: sandboxv1alpha1.Lifecycle{
		...
		sb.Status = sandboxv1alpha1.SandboxStatus{
			Service:       sandboxName,
			Replicas:      1,
