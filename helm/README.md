...
The chart embeds content generated by the `//go:generate` directives in [`codegen.go`](../codegen.go): the CRDs in `crds/`, and the controller-gen RBAC rules in `files/`. Regenerate them with `make fix-go-generate`.
