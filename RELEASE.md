...
- The Helm chart published as an OCI artifact to `oci://registry.k8s.io/agent-sandbox/charts/agent-sandbox` (stable `vX.Y.Z` tags only -- see [`docs/release.md`](docs/release.md#helm-chart-staging-and-promotion))

## Repository Release Flow

The project is released on an as-needed basis. The current process is:

1. Run `make release-promote TAG=vX.Y.Z` to create the repository tag, wait for the tagged image (and, for a stable `vX.Y.Z` tag, the Helm chart) to be pushed, and generate the promotion PR covering both. Creating the Git tag also triggers the Python SDK workflows.
1. Wait for the promotion PR to be approved and merged.
