...
func TestCreateSandboxPropagatesVolumeClaimTemplates(t *testing.T) {
	...
	template := &extensionsv1beta1.SandboxTemplate{
		...
		Spec: extensionsv1beta1.SandboxTemplateSpec{
			VolumeClaimTemplatesPolicy: extensionsv1beta1.VolumeClaimTemplatesPolicyOverrides,
			PodTemplate: sandboxv1beta1.PodTemplate{
				Spec: corev1.PodSpec{
...
func TestCreateSandboxClaimVolumeClaimTemplatesOverridesPolicy(t *testing.T) {
	scheme := newScheme(t)
	claimName := "vct-merge-claim"
	...
	claim := &extensionsv1beta1.SandboxClaim{
		...
		Spec: extensionsv1beta1.SandboxClaimSpec{
			...
			VolumeClaimTemplates: []sandboxv1beta1.PersistentVolumeClaimTemplate{
				{
					EmbeddedObjectMetadata: sandboxv1beta1.EmbeddedObjectMetadata{Name: "data"}, // Override template
					Spec: corev1.PersistentVolumeClaimSpec{
						AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
						Resources: corev1.VolumeResourceRequirements{
							Requests: corev1.ResourceList{
				...
				{
					EmbeddedObjectMetadata: sandboxv1beta1.EmbeddedObjectMetadata{Name: "custom"}, // Add new one
					Spec: corev1.PersistentVolumeClaimSpec{
						AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
						Resources: corev1.VolumeResourceRequirements{
							Requests: corev1.ResourceList{
	...
	template := &extensionsv1beta1.SandboxTemplate{
		...
		Spec: extensionsv1beta1.SandboxTemplateSpec{
			...
			VolumeClaimTemplates: []sandboxv1beta1.PersistentVolumeClaimTemplate{
				...
				{
					EmbeddedObjectMetadata: sandboxv1beta1.EmbeddedObjectMetadata{Name: "template-only"},
					Spec: corev1.PersistentVolumeClaimSpec{
						AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
						Resources: corev1.VolumeResourceRequirements{
							Requests: corev1.ResourceList{
	...
	sandbox := &sandboxv1beta1.Sandbox{}
	require.NoError(t, fakeClient.Get(context.Background(), types.NamespacedName{Name: claimName, Namespace: "default"}, sandbox))

	require.Len(t, sandbox.Spec.VolumeClaimTemplates, 3)

	getVCTByName := func(name string) *sandboxv1beta1.PersistentVolumeClaimTemplate {
		for _, v := range sandbox.Spec.VolumeClaimTemplates {
			if v.Name == name {
				return &v
			}
		}
	...
	dataVCT := getVCTByName("data")
	require.NotNil(t, dataVCT)
	require.True(t, dataVCT.Spec.Resources.Requests[corev1.ResourceStorage].Equal(resource.MustParse("5Gi")))

	templateOnlyVCT := getVCTByName("template-only")
	require.NotNil(t, templateOnlyVCT)
	require.True(t, templateOnlyVCT.Spec.Resources.Requests[corev1.ResourceStorage].Equal(resource.MustParse("10Gi")))

	customVCT := getVCTByName("custom")
	require.NotNil(t, customVCT)
	require.True(t, customVCT.Spec.Resources.Requests[corev1.ResourceStorage].Equal(resource.MustParse("2Gi")))
}

func TestCreateSandboxClaimVolumeClaimTemplatesDisallowedPolicy(t *testing.T) {
	scheme := newScheme(t)
	claimName := "vct-disallowed-claim"
	...
	template := &extensionsv1beta1.SandboxTemplate{
		ObjectMeta: metav1.ObjectMeta{Name: "vct-template", Namespace: "default"},
		Spec: extensionsv1beta1.SandboxTemplateSpec{
			VolumeClaimTemplatesPolicy: extensionsv1beta1.VolumeClaimTemplatesPolicyDisallowed,
			PodTemplate: sandboxv1beta1.PodTemplate{
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{Name: "app", Image: "test"}},
				},
	...
	_, err := reconciler.Reconcile(context.Background(), req)
	require.Error(t, err)
	require.ErrorIs(t, err, ErrVolumeClaimTemplatesDisallowed)
}

func TestCreateSandboxClaimVolumeClaimTemplatesAllowedPolicy(t *testing.T) {
	scheme := newScheme(t)
	claimName := "vct-allowed-claim"
	...
	require.Len(t, sandbox.Spec.VolumeClaimTemplates, 2)
	require.Equal(t, "data", sandbox.Spec.VolumeClaimTemplates[0].Name)
	require.Equal(t, "custom", sandbox.Spec.VolumeClaimTemplates[1].Name)
}

func TestCreateSandboxClaimVolumeClaimTemplatesAllowedPolicyForbiddenOverride(t *testing.T) {
	scheme := newScheme(t)
	claimName := "vct-allowed-override-claim"
	...
	claim := &extensionsv1beta1.SandboxClaim{
		...
		Spec: extensionsv1beta1.SandboxClaimSpec{
			...
			VolumeClaimTemplates: []sandboxv1beta1.PersistentVolumeClaimTemplate{
				{
					EmbeddedObjectMetadata: sandboxv1beta1.EmbeddedObjectMetadata{Name: "data"}, // Tries to override template VCT
					Spec: corev1.PersistentVolumeClaimSpec{
						AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
						Resources: corev1.VolumeResourceRequirements{
							Requests: corev1.ResourceList{
	...
	_, err := reconciler.Reconcile(context.Background(), req)
	require.Error(t, err)
	require.ErrorIs(t, err, ErrVolumeClaimTemplatesOverrideForbidden)
}

func TestCreateSandboxClaimVolumeClaimTemplatesBypassesWarmPool(t *testing.T) {
	scheme := newScheme(t)
	claimName := "vct-coldstart-claim"
	...
	// We seed a ready sandbox in the warm pool
	...
	fakeClient := fake.NewClientBuilder().WithScheme(scheme).
		WithObjects(claim, template, warmPool, readyWarmSandbox).
		WithStatusSubresource(claim).Build()

	warmSandboxQueue := queue.NewSimpleSandboxQueue()
	// Add warm-sandbox-1 to queue
	warmSandboxQueue.Add("vct-warmpool", queue.SandboxKey{Namespace: "default", Name: "warm-sandbox-1"})
	...
	// Since we bypassed the warm pool, the warm sandbox should NOT be adopted
	require.NoError(t, fakeClient.Get(context.Background(), types.NamespacedName{Name: "warm-sandbox-1", Namespace: "default"}, readyWarmSandbox))
	require.Nil(t, metav1.GetControllerOf(readyWarmSandbox))

	// Instead, a cold-started sandbox named claimName should be created
	sandbox := &sandboxv1beta1.Sandbox{}
	require.NoError(t, fakeClient.Get(context.Background(), types.NamespacedName{Name: claimName, Namespace: "default"}, sandbox))
