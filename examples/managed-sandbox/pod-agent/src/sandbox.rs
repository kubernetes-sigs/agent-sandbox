...
fn spawn_bwrap(
    ...
    // sandbox can't pull host kASLR symbols, keyring, or kmsg. We do
    // not mask /proc/self/mountinfo here: in nested kind/container
    // setups, mounting over that dynamic proc path fails with EPERM and
    // prevents the worker from starting.
    for path in ["/proc/kallsyms", "/proc/keys", "/proc/kcore", "/proc/kmsg"] {
