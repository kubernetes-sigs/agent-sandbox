...
* **Status: False** – The Sandbox is active, running, or in a transient lifecycle phase (`Running`, `PodTerminating`). `lastTransitionTime` is updated only when the status flips between `True` and `False`, and is retained while the status remains `False` even if the reason/message changes.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Provisioning** | `False` | `Running` | nil (Initial build) | **`False`** | `DependenciesNotReady` | Transient State: Sandbox is provisioning for the first time. |
...
| **Resuming** | `False` | `Running` | nil (Recreating) | **`False`** | `DependenciesNotReady` | Transient State: Sandbox resumption requested. Recreation of underlying workload is in progress. |
| **Operational** | `False` | `Running` | non-nil (Active) | **`True` / `False`** | `DependenciesReady` / `DependenciesNotReady` | Stable State: Sandbox is active and the underlying Pod has been provisioned. Note: The Pod might be Pending or Failed; we rely on `Ready` and `Finished` to track specific Pod health. |
