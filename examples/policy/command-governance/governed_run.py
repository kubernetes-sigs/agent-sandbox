...
# expansion (`$FOO`, `$(...)`, backticks) or a glob (`*?[`) that the shell
# resolves at runtime, not parse time. A token in the executable position that
# contains one of these could resolve to anything, including `rm --recursive
# --force` (`$(printf rm) --recursive --force /`, `/bin/r[m] -rf /`,
# `rm$IFS-rf$IFS/` with IFS word-splitting gluing the whole invocation into a
# single token). Treated as denied rather than evaluated, since evaluating
# shell expansion correctly means being a shell.
...
# bypass classification entirely. So _strip_wrapper fails closed instead.
_ENV_SPLIT_STRING_FLAGS = {"-S", "--split-string"}


def _strip_wrapper(tokens: list[str]) -> tuple[list[str], bool] | None:
    """Strips `sudo`/`env`/`xargs`/`command`/`nohup`/`nice`/`stdbuf` wrapper
    prefixes (and their own flags or env-var assignments) so e.g. `sudo rm -rf
    /` and `env rm -rf /` resolve to the same argv as `rm -rf /` for the
    checks below.
    ...
    while tokens:
        ...
        if exe == "env":
            ...
            while tokens and (
                ...
                if tokens[0] in _ENV_SPLIT_STRING_FLAGS:
                    return None
