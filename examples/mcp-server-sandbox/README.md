...
The `sha256 matches across pod restart` line is the proof that the PVC is
load-bearing — on `emptyDir` or the container overlay, the blob would be
gone after the Suspend→Resume cycle and `read_blob` would error with file-not-found.

### How the PVC is wired

`sandbox.yaml` declares one PVC under `spec.volumeClaimTemplates` and mounts
it inside the container at `/workspace`:
  ...
  podTemplate:
    spec:
      ...
      - name: mcp
        volumeMounts:
        - name: workspace
          mountPath: /workspace          # <-- the kernel mount point
...
- The Sandbox controller treats `volumeClaimTemplates` like a StatefulSet
  would — it instantiates a real `PersistentVolumeClaim` (and `PV`) tied
  to the Sandbox identity, not to any individual pod.
- The `volumeMounts` entry tells the kubelet to mount that PVC at
  `/workspace` inside the container.
- `mcp_server.py` does nothing PVC-aware: it just calls `path.write_bytes()` on `/workspace/<name>`. The kernel routes those bytes through the mount onto the PV, instead of onto the container's overlay filesystem.
- When the pod is destroyed (Suspended) and recreated (Running), the new
  pod gets the **same** PVC reattached. The container fs is fresh, but
  `/workspace` still contains everything that was written before — which
  is what Session 2 of the demo confirms.
