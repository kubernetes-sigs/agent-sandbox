...
*   **Decouple API from Runtime (Portable Backend)** `⏳ In Progress`
    *   Enable full customization of the runtime environment without breaking the API (via a common proto backend). The core runtime daemon `sandboxd` (`packages/sandboxd`) is implemented under KEP-539.2. TypeScript SDK runtime layer migration remains as the final piece. [[KEP #597](https://github.com/kubernetes-sigs/agent-sandbox/pull/597), [KEP #747](https://github.com/kubernetes-sigs/agent-sandbox/pull/747), [#977](https://github.com/kubernetes-sigs/agent-sandbox/issues/977)]
*   **SandboxTemplate & SandboxWarmPool Rolling Updates** `⏳ In Progress`
    *   Support rolling updates on WarmPools and Templates to update sandbox pods without causing downtime or service disruption. The `Recreate` update strategy is complete (issue [#764](https://github.com/kubernetes-sigs/agent-sandbox/issues/764)), while gradual rolling updates remain planned. [[#323](https://github.com/kubernetes-sigs/agent-sandbox/issues/323)]
...
*   **Typescript SDK Support** `⏳ In Progress`
    *   Implement high-level TypeScript SDK support for modern web application frontends. Provisioning capabilities are complete. Remaining sandboxd protocol layer runtime operations are tracked in [#977](https://github.com/kubernetes-sigs/agent-sandbox/issues/977).
...
*   **CI for PodSnapshot & AgentSandbox Regression Prevention** `⏳ In Progress`
    *   Introduce robust, isolated continuous integration tests to prevent regression. GKE Pod Snapshot examples/tests are manually verifiable under `examples/podsnapshot-golden-warmpool/`, but standard automated CI for snapshots remains in-progress.
...
*   **1st Class Router** `✅ Completed`
    *   Support the sandbox-router as a first-class citizen within the project (written in Go, built with the rest of the project, and shipping with out-of-the-box images). Added via the Go sandbox-router in [#838](https://github.com/kubernetes-sigs/agent-sandbox/pull/838) under `sandbox-router/`.
*   **Smart Warmpool Selection** `✅ Completed`
    *   Support intelligent warmpool matching and routing based on claim requirements. Implemented node-spread and FIFO matching in [#878](https://github.com/kubernetes-sigs/agent-sandbox/pull/878) (`extensions/controllers/queue/simple_sandbox_queue.go`). [[#491](https://github.com/kubernetes-sigs/agent-sandbox/issues/491)]
*   **Expand Python SDK Functionality** `✅ Completed`
    *   Natively support high-level convenience methods such as reading/writing files, executing commands (`run_code`), and interactive tools. Fully supports convenience methods (filesystem, command execution, and GKE snapshots).
*   **Agent Sandbox MCP (Model Context Protocol) Server** `✅ Completed`
    *   Integrate an MCP server endpoint via the router or SDK, making Agent Sandbox a native tool for MCP-enabled LLM runtimes. Added in [#1141](https://github.com/kubernetes-sigs/agent-sandbox/pull/1141) under `examples/mcp-server-sandbox/`.
*   **Extended Benchmarking & Better Performance** `✅ Completed`
    *   Benchmark large-scale workloads to identify performance bottlenecks, publish guidelines, and optimize controller throughput. Continuous benchmarking infrastructure is active in CI, and performance tuning guidelines are published in [docs/performance-tuning.md](docs/performance-tuning.md).
*   **Improve Claim Latency (200ms ➔ 100ms ➔ 50ms)** `✅ Completed`
    *   Analyze critical paths in the controller to reduce end-to-end sandbox assignment latencies down to sub-100ms. Sub-100ms p50 startup latencies have been achieved and documented.
*   **Storage Customization at Claim Time** `✅ Completed`
    *   Allow full custom storage volume and PVC sizing definitions directly in the SandboxClaim. Implemented volumeClaimTemplates support in [#960](https://github.com/kubernetes-sigs/agent-sandbox/pull/960). [[#225](https://github.com/kubernetes-sigs/agent-sandbox/issues/225), [#554](https://github.com/kubernetes-sigs/agent-sandbox/issues/554)]
*   **Strict Sandbox-to-Pod Mapping** `✅ Completed`
    *   Provide bulletproof, deterministic 1-to-1 mappings between a Sandbox claim and its backing Pod. Deterministic pod association and ownership checks were completed via [#127](https://github.com/kubernetes-sigs/agent-sandbox/pull/127) and verified via regression tests in `controllers/sandbox_controller_test.go`.
*   **Controller Custom Metrics** `✅ Completed`
    *   Track and expose standard metrics like sandbox creation latencies inside the controller. Implemented in `internal/metrics/metrics.go` and documented in [docs/metrics.md](docs/metrics.md). [[#125](https://github.com/kubernetes-sigs/agent-sandbox/pull/125)]
*   **Additional Prometheus Telemetry** `✅ Completed`
    *   Expose granular Prometheus counters to monitor API call frequencies, SDK usage, and overall controller performance. Implemented in `internal/metrics/metrics.go` and documented in [docs/metrics.md](docs/metrics.md).
*   **Integration with Ray (Rllib)** `✅ Completed`
    *   Seamless, high-performance container sandboxing for Ray training tasks. Reference implementation provided under [examples/ray-integration/](examples/ray-integration/).
*   **Integration with Agentic Frameworks** `✅ Completed`
    *   Provide native runtime execution environment plugins for LangChain, CrewAI, [OpenEnv](https://github.com/kubernetes-sigs/agent-sandbox/issues/132), kAgent and other tool-calling systems. Supports a rich set of client ecosystem integrations (LangChain, n8n, Aider, etc.) in `examples/`.
*   **Publish Benchmarking Methodology & Guides** `✅ Completed`
    *   Share systematic methodologies, configs, and reference results of running large-scale workloads. Guides and methodologies are published in [docs/performance-tuning.md](docs/performance-tuning.md) and [docs/testing.md](docs/testing.md).
...
### 📅 Roadmap Review Metadata
*   **Last Comprehensive Review:** September 2026
*   **Review Commitment:** This roadmap is reviewed and synchronized with the latest implementation status during each release cycle to ensure contributors can easily identify active scope and open areas of contribution.
