...
// errAdoptionConflict classifies a failure to complete a committed warm-pool
// adoption: the assigned sandbox was deleted, was won by another owner, or
// its optimistically locked adoption patch kept conflicting. It is expected
// contention, not a claim failure: the Ready condition surfaces it with a
// benign AdoptionConflict reason instead of a generic ReconcilerError, and
// the next pass re-enters or finishes the adoption from the converged view.
...
type SandboxClaimReconciler struct {
	...
	// cache. Used only to resolve an optimistic-lock conflict or a suspect
	// 404 against authoritative state, where the cache is stale by
...
// fresh claim base (from the authoritative reader — the cache is stale by
// definition when this runs), guarded so it only removes the exact reference
// it was asked to clean (a concurrent pass may already have moved it). The
// fresh object is copied back into claim so subsequent writes in this pass
// use the server-accepted base.
