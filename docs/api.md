...
| `ttlSecondsAfterCreated` _integer_ | ttlSecondsAfterCreated initializes lifecycle.shutdownTime to CreationTimestamp+TTL<br />when shutdownTime is unset.<br />A nil lifecycle is initialized with the Delete policy. An explicit shutdownTime<br />is never overwritten. |  | Minimum: 0 <br />Optional: \{\} <br /> |
