...
| `controller.leaderElectionNamespace` | Namespace for the leader election resource (auto-detected if empty). The chart does not generate namespace-specific leader-election RBAC; advanced users must configure it when using namespace-scoped RBAC. | `""` |
| `controller.watchNamespace` | Namespace(s) to watch; comma-separated for multi-namespace mode. Falls back to `WATCH_NAMESPACE` env var. Empty means cluster-scoped. When set, the chart injects `--enable-webhook=false` and `--leader-election-namespace=<release-namespace>`. ClusterRoles and ClusterRoleBindings are not rendered, and NOTES leaves workload and leader-election Roles and RoleBindings to advanced users. **Warning:** If etcd still contains v1alpha1 objects, migrate them to v1beta1 before enabling namespaced mode or keep a cluster-scoped conversion webhook available; API requests requiring conversion will otherwise fail. See [configuration](../docs/configuration.md#helm-behavior) for details. | `""` |
