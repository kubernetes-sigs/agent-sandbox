...
- `k8s/hardening/networkpolicy.yaml` → `config/network-policy/networkpolicy.yaml`
...
- `sandbox-router/deploy/*.yaml` → `config/sandbox-router/` (copied, then `hack/sync-k8s-manifests -router-dir` rewrites `metadata.namespace` and `subjects[].namespace` to `agent-sandbox-system` and regenerates `kustomization.yaml` as a resource list). The router image is remapped in `config/default/kustomization.yaml`, so `make bundle` can `kustomize edit set image` without breaking `verify-olm`. **Note:** `rbac-tokenreview.yaml` is copied for reference but excluded from the generated `kustomization.yaml` resources — it grants `system:auth-delegator` which is only needed when `--authz-mode=tokenreview` is enabled (the default is `allow-all`). `networkpolicy.yaml` is included and provides a working default: ingress is open on the proxy port (`--authz-mode` defaults to `allow-all`; set `tokenreview` or tighten `from:`), egress allows pods labeled `agents.x-k8s.io/sandbox-name-hash` on any port (caller-specified via `X-Sandbox-Port`) and the API server (needed because the OLM deployment ships `--cache-enabled=true`). See the comments in the file for tightening guidance.
