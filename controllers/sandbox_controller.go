...
func (r *SandboxReconciler) computeReadyCondition(sandbox *sandboxv1alpha1.Sandbox, err error, svc *corev1.Service, pod *corev1.Pod) metav1.Condition {
	...
	if pod != nil {
		...
		if sandbox.Spec.RunStrategy == sandboxv1alpha1.RunStrategyPaused {
			message = "Pod does not exist because sandbox is paused."
	...
	if sandbox.Spec.RunStrategy == sandboxv1alpha1.RunStrategyPaused {
		if pod != nil {
			if pod.ObjectMeta.DeletionTimestamp.IsZero() {
				log.Info("Deleting Pod because RunStrategy is Paused", "Pod.Namespace", pod.Namespace, "Pod.Name", pod.Name)
