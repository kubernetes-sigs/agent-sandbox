...
wrappers (`sudo rm -rf /`, including each wrapper's own arg-taking flags like
`sudo --user root`) — see `test_governed_run.py` for the full case list. A
command in the executable position that resolves via shell expansion or a
glob rather than being a literal spelling (`$(printf rm) --recursive --force
/`, `` `printf rm` --recursive --force / ``, `/bin/r[m] -rf /`, or IFS-glued
tokens like `rm$IFS-rf$IFS/`) is denied outright rather than evaluated, since
correctly resolving it would mean being a shell. Because `xargs` appends
