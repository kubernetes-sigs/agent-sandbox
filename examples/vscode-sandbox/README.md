...
### Harden Agent Sandbox isolation using Kata Containers and Hyper-V (Optional)
...
Hyper-V. There is no shared kernel between the host and the guest workload.

This example demonstrates how to use `Sandbox` along with CloudHypervisor in order
to utilize the lifecycle features of `Sandbox` in addition with hypervisor 
isolation features of CloudHypervisor + Hyper-V.
...
isolation (MSHV: Microsoft Hypervisor).
