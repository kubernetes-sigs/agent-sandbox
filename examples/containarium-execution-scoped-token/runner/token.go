...
// mintGatewayToken signs a run-scoped gateway token valid for ttl.
func mintGatewayToken(secret []byte, tenant, skillID, provider, runID string, ttl time.Duration) (mintedToken, error) {
	raw := make([]byte, 16)
